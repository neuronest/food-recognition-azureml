...
from utils import read_image_azure, read_text_azure
...
def init():
    ...
    global model
    global classes
    with open(join(os.getenv("AZUREML_MODEL_DIR"), PathsConfig.augmented_image_generator_path), 'rb') as fp:
        augmented_generator_schema = pickle.load(fp)
    with open(join(os.getenv("AZUREML_MODEL_DIR"), PathsConfig.image_generator_path), 'rb') as fp:
...
def run(raw_data):
    ...
    response = {"probabilities": y_pred_proba_list, "prediction_index": prediction_index, "prediction": prediction}
    return response
